...
Our FHIR POC is based on the R4 version of the HL7 FHIR specification. Reason for this is that also the EU Lab Reports FHIR IG is currently also defined in r4 and most of the Member States are using r5. If ever there is a need to migrate to r5, this can easily be done by updating the HAPI dependencies.

The decision was taken to not integrate the portal UI into the current OpenNCP portal reference implementation, but to start from scratch with the design of a portal that just serves the needs of the POC. Later it can be integrated in the OpenNCP portal reference implementation.

==== High level architecture

The high level setup for the technical POC consists of 4 parts:

*Portal*: User Interface and the entry point for the Health Professional to interact with the client. Implemented using JavaScript and the following tech stack: +
*Svelte*: reactive front-end library +
*Tailwind CSS* : CSS framework +
*Daisyui*: html components based on taiwindcss +
The build run on nodejs and using docker it run behind nginx as proxy server +
*FHIR Client*: The FHIR based application, accepting HTTP REST request from the portal and forwarding them to the Country of affiliation. This is the component to run in the Country of treatment and in the context of OpenNCP, it corresponds with the NCP-B. +
*FHIR Server*: The FHIR component that receives requests from the country of affiliation and forwards them to the national infrastructure to retrieve the requested resources. In the context of OpenNCP, it corresponds with the server deployed on NCP-A. +
*JPA HAPI FHIR server*: a FHIR server, using a database where FHIR resources are injected. This FHIR server is able to answer to requests issues by the FHIR server and can be considered as a mock for the national implementation that must be implemented on Member States side
...
For the POC we use docker as a container that comprises the different components and required (populated) databases. This allow all users to startup all the necessary components using a simple docker command.

Two parts are considered out of scope for the POC:

Connection with the SMP Server to retrieve the International Search Mask. This part is performed by the portal and is already done for OpenNCP.
Audit logging: in OpenNCP we currently use the OpenATNA component to generate and store audit logs. The OpenNCP gateway is used to access and filter these log files. This component can be reused as is or we could look at a more FHIR based solution using a Basic Audit Log Patterns (BALP) Interceptor where audit messages could be accessed using a FHIR API.
...
To retrieve a Patient resource, the /Patient REST API is used based on the PDQm (ITI-78) profile. The /Composition REST API is used to retrieve the different Compositions that match specific search criteria. The supported search parameters by the portal are the following:

* one or more patient identifier(s)
* study type
* date interval

When from the returned list of Compositions, a specific Laboratory Result Report needs to be returned, the $document operator is used on the composition to return the Bundle.

Also a translations-and-mappings module is implemented, accessible as a service or over a REST API. On its turn the translations-and-mappings uses the TSAM module to connect with the local terminology repository database (LTRDB) and containing the translations/mappings.
...
This part is not implemented in the POC.

We identified two different approaches

===== DICOM based audit events
All the transactions used by the Prototype require to generate audit events according the specifications provided by IHE and customized by MyHealth@EU project.

The non exhaustive list of transactions are:

Mobile Patient Demographics Query Transaction (ITI-78)
Internet User Authorization - get Token (IUA ITI-71)
Laboratory Result Report Transaction (profile not yet identified)
All these DICOM events are specified here: https://profiles.ihe.net/ITI/TF/Volume2/ITI-20.html#3.20.4.1.1.1

For this type of audit events, the ATNA module can be reused and the Gateway frontend can be used to search/filter audit events.
...
The advantage of this approach is that Audit logs can be searched/filtered using the FHIR REST API. And the audit logs could be used to reference back to the exchanged resources.
...
Input for the POC are the packages of the EU Lab Report FHIR IGs and the IPS FHIR IGs.
...
All the endpoints require an authentication of the client, as MyHealth@EU is using SAML Token https://webgate.ec.europa.eu/fpfis/wikis/x/AeTzN, the prototype will re-use the SAML Token Option https://profiles.ihe.net/ITI/IUA/index.html#372432-saml-token-option (_Please note that only one assertion must be passed into the security header in Fhir, a solution MUST be found to support the TRC and NoK token if needed into the Use Case_).
