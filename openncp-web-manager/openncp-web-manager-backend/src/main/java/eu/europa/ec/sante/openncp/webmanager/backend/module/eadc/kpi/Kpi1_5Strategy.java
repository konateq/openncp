...
@Component
public class Kpi1_5Strategy implements KpiStrategy {
    private final List<String> patientSummaryDataValues =
            List.of(
    ...
    public List<Transaction> filterTransactions(final List<Transaction> transactions) {
        return transactions.stream().filter(transaction ->
                CollectionUtils.isNotEmpty(transaction.getTransactionData())
                        && patientSummaryDataValues.contains(transaction.getTransactionData().get(0).getDataValue())).collect(Collectors.toList());
    }
}
