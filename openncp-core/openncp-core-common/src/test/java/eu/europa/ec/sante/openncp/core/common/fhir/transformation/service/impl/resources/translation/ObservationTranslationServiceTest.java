...
class ObservationTranslationServiceTest extends AbstractTranslationServiceTest {
    ...
    private IDomainTranslationService<ObservationResultsLaboratoryMyHealthEu> observationTranslationService;


    @BeforeEach
    void init() {
        observationTranslationService = new ObservationTranslationService(mockedTerminologyService);
    }
...
//        processTranslation(mockedTerminologyService, input.getCategory().get(1).getCoding().iterator().next(), "Bloedbankonderzoeken (set)");
        ...
        // Code
        processTranslation(mockedTerminologyService, input.getCode().getCoding().iterator().next(), "Albumine [massa/tijd] in 24-uurs urine");
